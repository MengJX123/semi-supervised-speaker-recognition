...
# Here we use a small ECAPA-TDNN, C=512. In my experiences, C=1024 slightly improves the performance but need more training time.
class ECAPA_TDNN(nn.Module):
    def __init__(self, C=1024, num_class=5994):
        super(ECAPA_TDNN, self).__init__()
        self.torchfbank = torch.nn.Sequential(
            PreEmphasis(),
            torchaudio.transforms.MelSpectrogram(sample_rate=16000, n_fft=512, win_length=400,
                                                 hop_length=160, f_min=20, f_max=7600, window_fn=torch.hamming_window, n_mels=80),
        )
        ...
        # the final layer
        #self.weight = nn.Parameter(torch.FloatTensor(num_class, 192), requires_grad=False)
